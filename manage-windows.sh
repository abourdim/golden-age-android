#!/usr/bin/env bash
# ============================================================================
#  What Muslims Gave the World Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/app-tier-s-golden-age}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.goldenage}"
AVD_NAME="${AVD_NAME:-golden_age_test}"
...
cmd_test() {
  ...
  local out="/tmp/golden-age-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   What Muslims Gave the World Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
